...
## CI/CD Project Setup and Verification

This section documents the four GitHub Actions workflows added under `.github/workflows/` and the exact steps required to configure and run them.

### 1. Project architecture

| Workflow | File | Trigger | Jobs |
|---|---|---|---|
| Frontend Continuous Integration | `.github/workflows/frontend-ci.yaml` | `pull_request` on `main`, paths `starter/frontend/**`, `workflow_dispatch` | `lint`, `test` (parallel) -> `build` |
| Backend Continuous Integration | `.github/workflows/backend-ci.yaml` | `pull_request` on `main`, paths `starter/backend/**`, `workflow_dispatch` | `lint`, `test` (parallel) -> `build` |
| Frontend Continuous Deployment | `.github/workflows/frontend-cd.yaml` | `push` on `main`, paths `starter/frontend/**`, `workflow_dispatch` | `lint`, `test` (parallel) -> `build_and_push` -> `deploy` |
| Backend Continuous Deployment | `.github/workflows/backend-cd.yaml` | `push` on `main`, paths `starter/backend/**`, `workflow_dispatch` | `lint`, `test` (parallel) -> `build_and_push` -> `deploy` |

The `build`/`build_and_push` job only runs if both `lint` and `test` succeed (`needs: [lint, test]`). CD's `deploy` job only runs after a successful `build_and_push`.
...
### 3. Required GitHub Repository Variable

Add this under **Settings -> Secrets and variables -> Actions -> Variables**:

- `REACT_APP_MOVIE_API_URL`

This is **not** used by Frontend CI (which always builds against `http://localhost:5000` for testing). It is required by Frontend CD, which builds the production frontend image against the real, deployed backend URL:

- CI value (hardcoded in `frontend-ci.yaml`): `http://localhost:5000`
- Production value (this repository variable): `http://<backend-load-balancer-hostname>` — the hostname of the `backend` Kubernetes `Service` (a `LoadBalancer`), obtained after Backend CD has deployed successfully. Do **not** append `/movies` — the React code in `src/components/MovieList.js` already requests `${REACT_APP_MOVIE_API_URL}/movies`, so the variable must hold only the base URL.

If this variable is not set, Frontend CD's `build_and_push` job fails fast with a clear error instead of silently building against `localhost`.
...
This provisions the VPC, the `cluster` EKS cluster, its node group, the `frontend` and `backend` ECR repositories, and the `github-action-user` IAM user. Record the outputs:

```bash
terraform output
```

Key outputs: `frontend_ecr`, `backend_ecr`, `cluster_name`, `cluster_version`, `github_action_user_arn`.

### 5. EKS access for the GitHub Actions IAM user
...
This grants the `github-action-user` IAM user access to the EKS cluster's Kubernetes RBAC so that the CD workflows can run `kubectl`/`kustomize` against it.

### 6. Deployment order (backend before frontend)

The frontend image must be built with the real backend URL baked in via `REACT_APP_MOVIE_API_URL`, so the backend must be deployed first:

1. `terraform apply` (infrastructure)
2. `./init.sh` (EKS access for the CI/CD IAM user)
3. Add the `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` secrets
4. Trigger **Backend Continuous Deployment** (push to `main` touching `starter/backend/**`, or `workflow_dispatch`)
5. `kubectl get service backend` — wait for `EXTERNAL-IP`/hostname to be assigned
6. `curl http://<backend-load-balancer>/movies` — confirm it returns the movie JSON
7. Set the `REACT_APP_MOVIE_API_URL` repository variable to `http://<backend-load-balancer>`
8. Trigger **Frontend Continuous Deployment**
9. `kubectl get service frontend` — retrieve its LoadBalancer hostname
10. Open `http://<frontend-load-balancer>` in a browser and confirm the movie list renders
11. When finished verifying, tear infrastructure down: `cd setup/terraform && terraform destroy`

### 7. Verifying the pipelines fail correctly

The rubric requires that broken code correctly fails CI. To exercise this locally before relying on it in CI:

```bash
# Frontend
cd starter/frontend
FAIL_TEST=true CI=true npm test   # expected: failure
FAIL_LINT=true npm run lint       # expected: failure

# Backend
cd starter/backend
FAIL_TEST=true pipenv run test    # expected: failure
pipenv run lint-fail              # expected: failure
```

Then re-run the normal commands (`npm run lint`, `CI=true npm test`, `pipenv run lint`, `pipenv run test`) and confirm they pass again. None of the workflows use `continue-on-error` or `|| true`, so a genuine lint/test/build/push/deploy failure will fail the corresponding GitHub Actions job.

### 8. Evidence / screenshots

See `evidence/README.md` for the exact list of screenshots to capture after a real deployment. No fabricated screenshots are included in this repository.
